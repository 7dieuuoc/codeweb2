!function(t) {
    ...
    ////////////////////
/* Modal - Video Quảng Cáo */
// Lấy các phần tử cần thiết
const modal = document.getElementById('adModal');
const video = document.getElementById('adVideo');
const closeBtn = document.getElementById('closeBtn');

// Danh sách video sẽ phát
const videos = [
    //'videow/vx10.mp4',
   
];

// Chỉ số của video hiện tại
let currentVideoIndex = 0;

// Hàm để phát video tiếp theo
function playNextVideo() {
    // Nếu đã phát hết tất cả video, đóng modal
    if (currentVideoIndex >= videos.length) {
        modal.style.display = 'none';  // Đóng modal
        return;
    }

    // Thay đổi video nguồn
    video.src = videos[currentVideoIndex];
    video.play();
    currentVideoIndex++;  // Tăng chỉ số video lên
}

// Khi video hiện tại kết thúc, tự động chuyển sang video tiếp theo
video.addEventListener('ended', playNextVideo);

// Khi bấm nút "X", đóng modal và dừng video
closeBtn.addEventListener('click', function() {
    modal.style.display = 'none';  // Đóng modal
    video.pause();  // Dừng video khi bấm X
    video.currentTime = 0;  // Quay lại đầu video
});

// Hiển thị modal khi lần đầu vào trang
window.onload = function() {
    modal.style.display = 'flex'; // Hiển thị modal
    playNextVideo();  // Bắt đầu phát video đầu tiên
};
...
//////////////////////////
    ...
    t.fn.prettyPhoto = function(o) {
        ...
        var p, a, s, n, l, r, d, h = this, c = !1, _ = t(window).height(), g = t(window).width();
        function m() {
            t(".pp_loaderIcon").hide(),
            projectedTop = scroll_pos.scrollTop + (_ / 2 - p.containerHeight / 2),
            ...
            $pp_pic_holder.animate({
                top: projectedTop,
                left: g / 2 - p.containerWidth / 2 < 0 ? 0 : g / 2 - p.containerWidth / 2,
                width: p.containerWidth
            }, settings.animation_speed, function() {
                $pp_pic_holder.find(".pp_hoverContainer,#fullResImage").height(p.height).width(p.width),
                ...
                settings.allow_expand && (p.resized ? t("a.pp_expand,a.pp_contract").show() : t("a.pp_expand").hide()),
            ...
            itemsPerPage = Math.floor((p.containerWidth - 100 - navWidth) / itemWidth),
            ...
            $pp_gallery_li.filter(":eq(" + set_position + ")").addClass("selected")) : $pp_pic_holder.find(".pp_content").off("mouseenter mouseleave"),
            o.ajaxcallback()
        }
        function f(e) {
        ...
        function u(t, e) {
            ...
            (r > g || l > _) && doresize && settings.allow_resize && !c) {
                for (resized = !0,
                fitting = !1; !fitting; )
                    r > g ? (imageWidth = g - 200,
                    imageHeight = e / t * imageWidth) : l > _ ? (imageHeight = _ - 200,
        ...
        function k(e) {
            ...
            settings.allow_expand && t("a.pp_expand").on("click", function(e) {
        ...
        return doresize = !0,
        scroll_pos = b(),
        t(window).off("resize.prettyphoto").on("resize.prettyphoto", function() {
            w(),
            _ = t(window).height(),
            g = t(window).width(),
            "undefined" != typeof $pp_overlay && $pp_overlay.height(t(document).height()).width(g)
        }),
        o.keyboard_shortcuts && t(document).off("keydown.prettyphoto").on("keydown.prettyphoto", function(e) {
        ...
        t.prettyPhoto.initialize = function() {
            ...
            pp_titles = isSet ? jQuery.map(h, function(e, i) {
                if (-1 != t(e).attr(settings.hook).indexOf(theRel))
                    return t(e).find("img").attr("alt") ? t(e).find("img").attr("alt") : ""
            }) : t.makeArray(t(this).find("img").attr("alt")),
            pp_descriptions = isSet ? jQuery.map(h, function(e, i) {
                if (-1 != t(e).attr(settings.hook).indexOf(theRel))
            ...
            k(this),
            settings.allow_resize && t(window).on("scroll.prettyphoto", function() {
                w()
            }),
            t.prettyPhoto.open(),
            !1
        ...
        t.prettyPhoto.open = function(e) {
            ...
            $pp_pic_holder.find(".currentTextHolder").text(set_position + 1 + settings.counter_separator_label + t(pp_images).length),
            ...
            $pp_pic_holder.fadeIn(function() {
                ...
                case "image":
                    ...
                    nextImage = new Image,
                    isSet && set_position < t(pp_images).length - 1 && (nextImage.src = pp_images[set_position + 1]),
                    prevImage = new Image,
                    isSet && pp_images[set_position - 1] && (prevImage.src = pp_images[set_position - 1]),
                    $pp_pic_holder.find("#pp_full_res")[0].innerHTML = settings.image_markup.replace(/{path}/g, pp_images[set_position]),
                    imgPreloader.onload = function() {
                        p = u(imgPreloader.width, imgPreloader.height),
                        m()
                    }
                    ,
                ...
                case "youtube":
                    ...
                    toInject = settings.iframe_markup.replace(/{width}/g, p.width).replace(/{height}/g, p.height).replace(/{wmode}/g, settings.wmode).replace(/{path}/g, movie);
                    break;
                case "vimeo":
                    p = u(movie_width, movie_height),
                    movie_id = pp_images[set_position];
                    var e = movie_id.match(/http(s?):\/\/(www\.)?vimeo.com\/(\d+)/);
                    movie = "//player.vimeo.com/video/" + e[3] + "?title=0&amp;byline=0&amp;portrait=0",
                    settings.autoplay && (movie += "&autoplay=1;"),
                    vimeo_width = p.width + "/embed/?moog_width=" + p.width,
                    toInject = settings.iframe_markup.replace(/{width}/g, vimeo_width).replace(/{height}/g, p.height).replace(/{path}/g, movie);
                    break;
                case "quicktime":
                    (p = u(movie_width, movie_height)).height += 15,
                    p.contentHeight += 15,
                    p.containerHeight += 15,
                    toInject = settings.quicktime_markup.replace(/{width}/g, p.width).replace(/{height}/g, p.height).replace(/{wmode}/g, settings.wmode).replace(/{path}/g, pp_images[set_position]).replace(/{autoplay}/g, settings.autoplay);
                    break;
                case "flash":
                    p = u(movie_width, movie_height),
                    ...
                    toInject = settings.flash_markup.replace(/{width}/g, p.width).replace(/{height}/g, p.height).replace(/{wmode}/g, settings.wmode).replace(/{path}/g, filename + "?" + flash_vars);
                    break;
                case "iframe":
                    p = u(movie_width, movie_height),
                    frame_url = pp_images[set_position],
                    frame_url = frame_url.substr(0, frame_url.indexOf("iframe") - 1),
                    toInject = settings.iframe_markup.replace(/{width}/g, p.width).replace(/{height}/g, p.height).replace(/{path}/g, frame_url);
                    break;
                case "ajax":
                    doresize = !1,
                    p = u(movie_width, movie_height),
        ...
        t.prettyPhoto.changePage = function(e) {
            ...
            set_position < 0 && (set_position = t(pp_images).length - 1)) : "next" == e ? (set_position++,
            set_position > t(pp_images).length - 1 && (set_position = 0)) : set_position = e,
        ...
        t.prettyPhoto.startSlideshow = function() {
            void 0 === d ? ($pp_pic_holder.find(".pp_play").off("click").removeClass("pp_play").addClass("pp_pause").on("click", function() {
        ...
        this.off("click.prettyphoto").on("click.prettyphoto", t.prettyPhoto.initialize)
    }
}(jQuery);
